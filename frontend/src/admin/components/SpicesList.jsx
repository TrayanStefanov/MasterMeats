...
const SpiceList = ({ onEdit }) => {
  ...
  const handleIncreaseStock = async (spiceId) => {
    ...
    await updateSpice(spiceId, { stockInGrams: spice.stockInGrams + increment });
  ...
  if (loading && spices.length === 0)
    return <p className="text-center py-8 text-secondary/60">{tCommon("loading.loading")}</p>;

  if (!spices || spices.length === 0)
    return (
      <>
        <SpiceFilters />
        <p className="text-center py-8 text-secondary/60">{tProduction("spices.empty", "No spices found.")}</p>
      </>
    );
      ...
      >
        <table className="min-w-full divide-y divide-accent-content">
          <thead className="bg-secondary/80 font-semibold text-primary uppercase text-xs tracking-wider">
            <tr>
            <th className="px-6 py-3 text-left">{tProduction("spices.list.name")}</th>
              <th className="px-6 py-3 text-left">{tProduction("spices.list.supplier")}</th>
              <th className="px-6 py-3 text-left">{tProduction("spices.list.costPerKg")}</th>
              <th className="px-6 py-3 text-left">{tProduction("spices.list.notes")}</th>
              <th className="px-6 py-3 text-left">{tProduction("spices.list.stock")}</th>
              <th className="px-6 py-3 text-right">{tProduction("spices.list.actions")}</th>
            </tr>
          </thead>
          ...
          <tbody className="bg-accent/70 divide-y divide-accent-content">
            {spices.map((spice) => (
              <Fragment key={spice._id}>
                <tr
                  className={`transition-colors cursor-pointer ${getRowClass(spice._id)}`}
                  onClick={() =>
                    setExpandedSpice(expandedSpice === spice._id ? null : spice._id)
                  }
                >
                  <td className="px-6 py-4 text-secondary font-medium">{spice.name}</td>
                  <td className="px-6 py-4 text-secondary/70">{spice.supplier || "—"}</td>
                  ...
                  <td className="px-6 py-4 text-secondary/70">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setStockInputs((prev) => ({ ...prev, [spice._id]: e.target.value }))
                        }
                  ...
                  <td className="px-6 py-4 text-right flex justify-end gap-2">
                    ...
                    <button
                      ...
                      className={`px-2 py-0.5 text-xs rounded ${spice.isActive ? "bg-green-600" : "bg-red-600"} text-white`}
                    >
